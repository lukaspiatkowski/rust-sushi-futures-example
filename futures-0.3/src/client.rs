...
pub fn get_clients(num_of_clients: usize) -> impl Stream<Item = Client> {
    stream::unfold(num_of_clients, async move |clients_left| {
        if clients_left > 0 {
            let mut gen = StdThreadGen::new(10);
            Some((Client::arbitrary(&mut gen), clients_left - 1))
        } else {
            None
        }
    })
